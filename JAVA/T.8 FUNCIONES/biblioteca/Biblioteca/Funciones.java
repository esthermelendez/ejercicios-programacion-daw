...
public class Funciones {
   ...
   * @return posición de la primera ocurrencia del dígito dentro del número o -1
   * si no se encuentra
   */
  ...
  public static long decimalABinario(int decimal) {
    ...
    while (decimal > 1) {
      binario = Biblioteca.Funciones.pegaPorDetras(binario, decimal % 2);
      decimal = decimal / 2;
    }
    binario = Biblioteca.Funciones.pegaPorDetras(binario, 1);
    binario = Biblioteca.Funciones.voltea(binario);
    binario = Biblioteca.Funciones.quitaPorDetras(binario, 1);
  ...
  public static long octalABinario(long octal) {
    ...
    for (int i = 0; i < digitos(octal); i++) {
      binario = binario * 1000 + decimalABinario(digitoN(octal, i));
    }
